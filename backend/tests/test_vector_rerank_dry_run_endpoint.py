...
class TestVectorRerankDryRunEndpoint(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_dry_run_calls_external_rerank_and_never_returns_plain_api_key(self) -> None:
        fake = _FakeClient(
            resp=_FakeResponse(
                {
                    "results": [
        ...
        self.assertEqual(obs.get("hybrid_alpha"), 0.33)
        ...
        self.assertEqual(fake.last_post["headers"], {"Authorization": "Bearer rk-test-SECRET1234"})
        self.assertEqual(
            fake.last_post["json"],
