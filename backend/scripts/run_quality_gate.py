...
def main() -> int:
    ...
    # 3. 真实空库迁移 + metadata drift 守门（调用方创建并在 finally 清理临时 SQLite 数据库）。
    contract_db = BACKEND_ROOT / ".quality-alembic-contract.db"
    contract_db.unlink(missing_ok=True)
    try:
        run_step("scripts/check_alembic.py", "--database-url", "sqlite:///./.quality-alembic-contract.db")
    finally:
        contract_db.unlink(missing_ok=True)
    # 4. 测试安全网 + 覆盖率基线：只跑"本该绿"的测试（排除 known_issue 已知 bug）。
